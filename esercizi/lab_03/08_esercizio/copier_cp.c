...
int main(int argc, char** argv){
	if(argc != 3){
		fprintf(stderr, "[copier] - Errore: Sono necessari i due path dei file da copiare\n");
		exit(1);
	}
	...
	if(first_fp == NULL || second_fp == NULL){
		fprintf(stderr, "[copier] - Errore: Uno dei due file non e' stato aperto correttamente!\n");
		exit(1);
	}
