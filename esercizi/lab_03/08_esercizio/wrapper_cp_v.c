...
int main(){
	...
	switch(fork()){
		...
		case 0:
			...
			char* copier_params[4] = { copier_path, "./files/source.txt", "./files/dest.txt", NULL };
		...
		default:
			...
			if(children_pid <= 0){
				fprintf(stderr, "%s PARENT - the wait(); command returned an error!\n", prog_name);
				exit(1);
			}

			if(children_stat == 1){
				fprintf(stderr, "%s PARENT - the children process terminated with an error!\n", prog_name);
				exit(1);
			}
