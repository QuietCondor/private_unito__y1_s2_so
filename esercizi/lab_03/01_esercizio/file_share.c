...
int main(){
	...
	if (fp == NULL){
		fprintf(stderr, "Errore: non e' stato possibile aprire il file nel path %s\n", shared_file_path);
		return 1;
	}
	...
	switch(pid){
		...
		case -1:
			fprintf(stderr, "La fork del processo figlio e' andata in errore\n");
		...
		case 0:
			...
			if (children_fp == NULL){
				fprintf(stderr, "Errore: il children non e' in grado di aprire il file nel path '%s'", shared_file_path);
				exit(1);
			}

			// poi scrivo qualcosa
			fprintf(children_fp, "Children scrive a padre\nChildren scrive a padre passo.");
		...
		default:
			...
			if(wait_result == -1){
				fprintf(stderr, "La wait al processo figlio e' andata in errore!\n");
				exit(1);
			}
			...
			// una volta aspettato, deve andare a stampare a video il risultato della scrittura del file
			while(!feof(fp)){
