...
void custom_signal_handler(int signal){
	...
	// NB: per usare sigdescr_np va aggiunto _GNU_SOURCE come macro prima di ogni include
...
int main(){
	...
	// - signal(int signal, void (*disposition)(int)) => non tanto consigliata perche'
	//   varia nel comportamento. Restituisce il puntatore alla disposizione precedente
	...
	do{
		...
		printf("\t2. use sigaction(int, *struct sigaction new, *struct sigaction old);\n");
		printf("\t0. exit\n");
		...
		switch(choice){
			...
			case 1: // use signal();
				...
				void (*old_handler)(int) = signal(signal_to_handle, new_handler);
				
				// signal(); returns SIG_ERR on error and populates it in errno
				if (old_handler == SIG_ERR){
					fprintf(stderr, "ERROR: could not set the signal handler with signal()!\n");
					fprintf(stderr, "\tError details: \n\t\tError name: %s\n\t\tError desc: %s\n", strerrorname_np(errno), strerror(errno));
				} else {
					printf("signal handler for signal 'SIG%s': '%s' set!\n", sigabbrev_np(signal_to_handle), strsignal(signal_to_handle));
					printf("try to send a signal with the system application kill -SIG%s\nNow waiting..\n", sigabbrev_np(signal_to_handle));
					pause();
				}
				break;
			case 2: // use sigaction();
				...
				int action_res = sigaction(signal_to_handle, &new_action, &old_action);
