...
int main() {
    ...
    switch(id_children){
        ...
        default: // father
            ...
            if (kill_res == 0){
                printf("[parent]: the kill of the child process with id %u has been concluded successfully!\n", id_children);
            } else {
                char* error = strerror(errno);
                fprintf(stderr, "[parent]: the kill of the process has resulted in an error!: '%s'\n", error);
                return -1;
            }
            break;
    }
