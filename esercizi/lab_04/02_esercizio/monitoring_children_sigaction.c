...
int main(){
	...
	switch(child_pid){
		case -1: // error
			fprintf(stderr, "Error: could not create children process with fork();! '%s'\n", error);
			break;
		case 0: // children
			...
			child_disposition.sa_handler = custom_terminal_handler; // adding custom handler
			
			// defining the mask
			sigset_t mask;
			int mask_result = sigemptyset(&mask);
			if (mask_result != 0) {
				fprintf(stderr, "Error: could not correctly initialize the mask for the signal handler! '%s'\n", strerror(errno));
				wait(NULL);
				exit(1);
			}
			...
			printf("[children]: signal handler setted! try it! Now waiting for signal..\n");

			// waiting for signal
			pause();
			printf("[children]: signal received! Now reverting to default and waiting again!\n");
